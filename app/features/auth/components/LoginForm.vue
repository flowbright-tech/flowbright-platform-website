  ...
  <div class="w-full max-w-md mx-auto">
    <UCard class="glass-panel shadow-2xl ring-1 ring-slate-200/50 dark:ring-slate-800/80 rounded-2xl">
      ...
      <form @submit.prevent="handleLogin" novalidate class="space-y-6">
        <!-- API Error Alert -->
        <div v-if="errors.api"
          class="p-3.5 rounded-xl bg-rose-50 dark:bg-rose-950/35 border border-rose-200/60 dark:border-rose-800/80 text-xs text-rose-700 dark:text-rose-300 flex items-center gap-2.5">
        ...
        <UFormField :label="$t('auth.email')" :error="errors.email ? $t(errors.email) : undefined">
          <UInput v-model="form.email" type="email" :placeholder="$t('auth.email_placeholder')"
            icon="i-heroicons-envelope" size="lg" class="w-full font-medium" @input="errors.email = ''" />
        </UFormField>
        ...
        <div class="space-y-1">
          <UFormField :label="$t('auth.password')" :error="errors.password ? $t(errors.password) : undefined">
            <UInput v-model="form.password" :type="showPassword ? 'text' : 'password'"
              :placeholder="$t('auth.password_placeholder')" icon="i-heroicons-lock-closed" size="lg"
              class="w-full font-medium" @input="errors.password = ''">
              <template #trailing>
                <UButton color="gray" variant="ghost" :icon="showPassword ? 'i-heroicons-eye-slash' : 'i-heroicons-eye'"
                  size="sm" class="mr-1" @click="showPassword = !showPassword" />
              </template>
            </UInput>
          </UFormField>
        ...
        <div v-if="cleansedNotice"
          class="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/35 border border-emerald-200/60 dark:border-emerald-800/80 text-xs text-emerald-700 dark:text-emerald-300 flex items-center gap-2.5">
