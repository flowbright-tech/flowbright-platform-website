  ...
  <UCard class="glass-panel h-full flex flex-col">
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <UBadge color="amber" variant="subtle" size="xs" class="font-bold">
          TOP SALES
        </UBadge>
      </div>
    </template>

    <div v-if="!packages || packages.length === 0" class="py-8 text-center text-slate-400 text-sm">
    ...
    <div v-else class="space-y-4 my-auto">
      <div
        ...
        :key="item.package_id"
        class="p-3 rounded-xl border border-slate-200/60 dark:border-slate-800/80 bg-white/40 dark:bg-slate-900/40 hover:bg-indigo-50/30 dark:hover:bg-slate-800/40 transition-colors"
      >
        <div class="flex items-center justify-between mb-1.5">
          <div class="flex items-center gap-2.5 min-w-0">
            <span
              class="w-6 h-6 rounded-lg flex items-center justify-center text-xs font-black shrink-0"
          ...
          <span class="text-xs font-semibold text-slate-500 dark:text-slate-400 shrink-0 ml-2">
        ...
        <div class="flex justify-between items-center mt-2 text-xs">
          <span class="text-slate-400 dark:text-slate-500">Revenue</span>
