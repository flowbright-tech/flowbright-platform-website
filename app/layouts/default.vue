<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col antialiased">
      ...
      <div class="flex items-center gap-2 sm:gap-3">
        ...
        <UDropdownMenu :items="userDropdownItems" :content="{ align: 'end' }">
          <UButton color="gray" variant="ghost" size="md" class="flex items-center gap-2 p-1 sm:px-2">
            <UAvatar :alt="displayName" size="2xs" chip-color="emerald" class="ring-2 ring-indigo-500/30" />
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <aside
        class="hidden lg:flex flex-col w-64 border-r border-slate-200/80 dark:border-slate-800/80 bg-white/50 dark:bg-slate-900/40 backdrop-blur-md p-4 space-y-6">
          ...
          <div class="p-6 flex-1 flex flex-col justify-between h-full bg-white dark:bg-slate-900">
            <div class="space-y-6">
              <div class="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
                <NuxtLink :to="localePath('/')" class="inline-flex items-center focus:outline-hidden"
                  @click="isMobileMenuOpen = false">
                  <div
                    class="flex items-center px-2.5 py-1 rounded-xl bg-transparent dark:bg-transparent">
                    <img src="/logo.svg" alt="SRP" class="h-7 sm:h-8 w-auto object-contain" />
                  </div>
                </NuxtLink>
              ...
              <div
                class="p-3 rounded-xl bg-indigo-500/10 text-xs font-semibold text-indigo-600 dark:text-indigo-400 truncate">
                {{ companyName }}
              </div>
            ...
            <UButton color="rose" variant="soft" block icon="i-heroicons-arrow-right-on-rectangle"
              @click="handleLogout">
              {{ $t('auth.logout') }}
            </UButton>
          </div>
    ...
    <ConfirmModal v-model:open="isLogoutConfirmOpen" :title="$t('auth.confirm_logout_title')"
      :description="$t('auth.confirm_logout_desc')" confirm-color="primary"
      confirm-icon="i-heroicons-arrow-right-on-rectangle" :confirm-text="$t('auth.logout')" @confirm="confirmLogout" />
  </div>
</template>
...
const { session, activeTenant, logout, user, company } = useAuthEngine()
...
const companyName = computed(() => {
  ...
  return activeTenant.value?.name || 'SRP Company'
})
