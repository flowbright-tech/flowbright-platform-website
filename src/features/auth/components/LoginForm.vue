...
const { t } = useI18n()
const { tenants, login } = useAuthEngine()
...
const handleLogin = async () => {
  ...
  try {
    ...
    // Save token inside local session state, assign first tenant by default
    login(form.email, data.token || data.data?.token || 'mock_jwt_token_auth', tenants.value[0].id)
