    ...
    <UCard class="glass-panel overflow-hidden" :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      ...
      >
        ...
        <template #category-data="{ row }">
          <UBadge
            :color="
              row.category === 'raw_materials' ? 'amber' :
              row.category === 'it_hardware' ? 'indigo' :
              row.category === 'logistics' ? 'emerald' : 'sky'
            "
            variant="subtle"
            size="xs"
            class="capitalize font-semibold"
          >
            {{ $t(`vendors.cat_${row.category}`) }}
          </UBadge>
        </template>
...
const columns = computed(() => [
  { key: 'name', label: t('vendors.col_name') },
  { key: 'taxId', label: t('vendors.col_tax_id') },
  { key: 'regId', label: t('vendors.col_reg_id') },
  { key: 'category', label: t('vendors.col_category') },
  { key: 'actions', label: t('vendors.col_actions'), class: 'text-right' }
])
</script>
